...
#This class is used to create a dataset for each of the training, validation, and test sets
class Dataset(torch.utils.data.Dataset):
    ...
    # This method returns an item in the dataset at index i. The item returned in the test set
    # includes the type for each claim as well for the final accuracy calculation by claim type
    def __getitem__(self, i):
        ...
        if self.split == "test":
            if "negation" in self.types[i]:
                ...
                raise ValueError()
            #Here the value of key "e" is a tuple of 2 strings:
            #("entity1,rel,entity2,..,entityn|entity1,rel,entity2,..,entityn|...", similar string)
            #The 1st string in the tuple is for connected path, and second for walkable path
            sample = {
...
# The DataCollator class handles how each batch is formed. The__call__method calls the batchfy 
# method to create a batch. More details above the batchfy method
...
class DataCollator:
    ...
    # The features is a list of batch items. The batch is returned as a dictionary
...
#Loading the KG path and the data path from the command line arguments
data_path = args.data_path
kg_path = args.kg_path

#prepare_input's output dump-> 
# 3 dictionaries, 1 each for train,dev,test of format:
#{claim->{connected"[],"walkable":[]} }
...
#Load the train_candid_paths file as saved in the preprocess file
...
#train_claims->[claim]
#train_labels->[[label]]
#train_evis:[[connected list, walkable list]]
...
#load the dev_candid_paths.bin file as saved in the preprocess dictionary
...
#dev_claims->[claim]
#dev_labels->[[label]]
#dev_evis:[[connected list, walkable list]]
dev_claims = list()
dev_evis = list()
dev_labels = list()
...
#Load the test_candid_paths_top3.bin file as saved in the preprocess.py file
...
#test_claims->[claim]
#test_labels->[[label]]
#test_evis:[[connected list, walkable list]]
#test_types is the value of the types key for a particular claim in the test set
test_claims = list()
test_evis = list()
test_labels = list()
test_types = list()
...
#Initializing the dataset and DataCollator for training set
...
train_loader = torch.utils.data.DataLoader(
    train_dataset, 
    shuffle=True, 
    batch_size=16,#was 32 before I changed 
    collate_fn=collator, 
    drop_last=True,
    num_workers=0, 
    pin_memory=False
...
# This classifier is for classifying a claim in the test set with where each claim is concatenated
# with the list of paths for each entity in the claim
...
# This classifier is for classifying a claim in the test set with where each claim is not
# concatenated with the list of paths for each entity in the claim
...
# Creating a dictionary for sentence classifier and concatenated claim classifier and transferring the classifier to
# a GPU
model = {
...
# This is the training loop, this implements early stopping: if the development accuracy does not improve after 3 
# epochs, then the training terminates 
for epoch in range(args.epoch):
    ...
    file_name = "trainTime.txt"
    # Open the file in write mode
    with open(file_name, "w") as file:
        file.write(f"Total time taken: {totalTime}")
    ...
    if best < accuracy:
        ...
        torch.save(model.state_dict(), f'modelCheckpoint{epoch}.bin')
        stop_counter = 0
    else:
        stop_counter += 1
    if stop_counter > 3:
...
#This is the evaluation loop for the test set. The final predictions for the test set are generated here
t1=time.time()
with torch.no_grad():
    scores = list()
    rtypes = list()
    ...
    for rt in total_rtype.unique():
        ...
        rtdict[str(rt.item())] = total_score[idcs.cpu()].mean().item()
    accuracy = f"{torch.cat(scores).float().mean().item():.4f}"
